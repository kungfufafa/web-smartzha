            ...
            <div class="row mb-2">
                <div class="col-6">
                    <div class="info-box bg-transparent shadow-none">
                        <img src="<?= base_url() ?>/assets/img/smartzha_circle.png" width="60" height="60" alt="Smartzha">
                        <div class="info-box-content">
                                <span class="text-white"
                                      style="font-size: 24pt; line-height: 0.7;"><b>SMARTZHA</b></span>
                            <span class="text-white">Smartzha CBT</span>
                        </div>
                    </div>
                </div>
                <div class="col-6">
...
<!-- Audio warning untuk deteksi kecurangan -->
<audio id="warning-sound" src="<?= base_url() ?>assets/app/audio/beep-warning-6387.mp3" preload="auto"></audio>
...
<script>
    ...
    document.addEventListener("visibilitychange", () => {
        if (document.hidden && infoJadwal.reset_login === '1') {
            // Play warning sound when cheating detected
            var warningSound = document.getElementById('warning-sound');
            warningSound.currentTime = 0;
            warningSound.play();
            
            // Wait for audio to finish playing before redirect
            warningSound.onended = function() {
                location.href=base_url+"siswa/leavecbt/<?= $jadwal->id_jadwal ?>/<?= $siswa->id_siswa ?>";
            };
        }
    });

    // Deteksi split screen untuk HP
    var initialHeight = window.innerHeight;
    var initialWidth = window.innerWidth;

    window.addEventListener('resize', function() {
        if (infoJadwal.reset_login !== '1') return;
        
        var heightRatio = window.innerHeight / initialHeight;
        var widthRatio = window.innerWidth / initialWidth;
        
        // Split screen: tinggi atau lebar berkurang signifikan (< 70%)
        if (heightRatio < 0.7 || widthRatio < 0.7) {
            var warningSound = document.getElementById('warning-sound');
            warningSound.currentTime = 0;
            warningSound.play();
