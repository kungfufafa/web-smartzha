                ...
                <div class="card-body">
                    <?php if ($existing_log && $existing_log->jam_masuk): ?>
                        <div class="alert alert-success text-center">
                            <h4><i class="fas fa-check-circle"></i> Sudah Absen Masuk</h4>
                            <p class="mb-0">
                        ...
                        <div class="text-center">
                            <?php if ($config->require_photo): ?>
                                <div class="alert alert-warning">
                                    <i class="fas fa-camera mr-2"></i>Foto selfie wajib untuk absen masuk.
                                </div>
...
function doCheckIn(lat, lng, qrToken) {
    ...
    if (requirePhoto && !photoFile) {
        alert('Foto selfie wajib untuk absen masuk');
        var btn = document.getElementById('btn-checkin');
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-location-arrow"></i> Ambil Lokasi & Masuk';
    ...
    .then(function(result) {
        if (result.success) {
            ...
            alert('Absen Masuk gagal: ' + result.message);
...
function submitCheckout(lat, lng, qrToken) {
    ...
    .then(function(result) {
        if (result.success) {
            ...
            alert('Absen Pulang gagal: ' + result.message);
